			    ...
			    LPOVERLAPPED lpOverlapped)
{
	...
	if (dwErrorCode == ERROR_SUCCESS) {
		do {
			pNotify =
			    (PFILE_NOTIFY_INFORMATION)&pWatch->mBuffer[offset];
			offset += pNotify->NextEntryOffset;

#if defined(UNICODE)
			{
				lstrcpynW(
				    szFile, pNotify->FileName,
				    min(MAX_PATH, pNotify->FileNameLength /
							  sizeof(WCHAR) +
						      1));
			}
#else
			...
			pWatch->mFileWatcher->handleAction(pWatch, szFile,
							   pNotify->Action);
...
WatchStruct *CreateWatch(LPCTSTR szDirectory, bool recursive,
			 DWORD mNotifyFilter)
{
	WatchStruct *pWatch;
	size_t ptrsize = sizeof(*pWatch);
	pWatch = static_cast<WatchStruct *>(
...
//--------
FileWatcherWin32::FileWatcherWin32() : mLastWatchID(0) {}
				   ...
				   FileWatchListener *watcher, bool recursive)
{
	...
	WatchStruct *watch =
	    CreateWatch(directory.c_str(), recursive,
...
//--------
void FileWatcherWin32::handleAction(WatchStruct *watch, const String &filename,
